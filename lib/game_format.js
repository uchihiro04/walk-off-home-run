...
module.exports = class GameFormat {
  constructor() {
  ...
  async playGame() {
    ...
    const wins = await this.#playChampionship(highSchool);
  ...
  async #decideName() {
    ...
    return gameData.nameOfHighSchool;
  }
  ...
  #explain(highSchool) {
    ...
    if (playCount > 1) {
      ...
      const championshipCount = wins.filter((win) => win === 3).length;
      ...
      if (championshipCount > 0) {
        console.log(
          `今までに${championshipCount}回の優勝を経験した${highSchool}高校。甲子園出場${playCount}回目となる今年、果たして優勝を手にすることはできるのか……。\n`
        );
      }
    }
  ...
  async #playChampionship(highSchool) {
    let wins = 0;
    const firstRound = await this.championship.playFirstRound();
    if (!firstRound) {
    ...
    wins++;
    const semifinals = await this.championship.playSemifinals(highSchool);
    if (!semifinals) {
      this.#createResult(wins);
      return wins;
    }
    ...
    wins++;
    const final = await this.championship.playFinal(highSchool);
    if (!final) {
      this.#createResult(wins);
      return wins;
    } else {

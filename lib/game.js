...
module.exports = class Game {
  ...
  deleteData() {
    if (!fs.existsSync(this.gameData.dir)) {
      console.log("データがありません");
      return;
    }
    fs.rm(this.gameData.dir, { recursive: true }, (err) => {
      if (err) throw err;
  ...
  async #playChampionship(highSchool) {
    ...
    wins++;
    const semifinals = await playSemifinals(highSchool);
    if (!semifinals) {
      this.#createResult(wins);
      return wins;
    }
    ...
    wins++;
    const final = await playFinal(highSchool);
    if (!final) {
      this.#createResult(wins);
      return wins;
    } else {
